	...
	void FixedUpdate () {
				...
				if (!PlayerAttack.inAtk) { // Pausar movimento quando estiver em ataque
						if (Input.GetKey (KeyCode.UpArrow)) {
								GetComponent<Rigidbody2D>().AddForce (new Vector3 (0, 1, 0) * maxSpeed * Time.deltaTime);
								float w = 0; 
								float h = 1;
								...
								if (Input.GetKey (KeyCode.RightArrow)) {
										GetComponent<Rigidbody2D>().AddForce (new Vector3 (1, 0, 0) * maxSpeed * Time.deltaTime);
								}
